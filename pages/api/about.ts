...
import {About, PersonalInfo} from '@/constant/FirebaseData'
...
const handleGet = async (_req: NextApiRequest, res: NextApiResponse) => {
    try{
        const aboutSnapshot = await db.ref('about').once('value')
        const aboutData = aboutSnapshot.val() as Record<string, Omit<About, 'uid'>> | null;
        const hero: About[] = aboutData
            ? Object.entries(aboutData).map(([uid, data]) => ({uid, ...data})) : []

        const personalInfoSnapshop = await db.ref('personalInfo').once('value')
        const personalData = personalInfoSnapshop.val() as Record<string, PersonalInfo> | null
